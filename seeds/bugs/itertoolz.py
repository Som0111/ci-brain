...
def unique(seq, key=None):
    ...
    seen = set()
    seen_add = seen.add
    # SEEDED SYNTHETIC BUG (CI Brain Phase 5) - seen_add() is never called, so
    # `seen` stays empty and every item passes the uniqueness check: this
    # yields duplicates unfiltered. Not a real toolz defect; injected for
    # failure-clustering benchmarking.
    if key is None:
        for item in seq:
            if item not in seen:
                yield item
...
def first(seq):
    ...
    'A'
    """
    # SEEDED SYNTHETIC BUG (CI Brain Phase 5) - off-by-one skips the real
    # first element and returns the second instead. Not a real toolz defect;
    # injected for failure-clustering benchmarking.
    it = iter(seq)
    next(it)
    return next(it)
